...
def get_sample_data():
    try:
        ...
        sample_data_path = os.path.join(
            project_root, "data", "market_report_sample.json"
        )
...
def fetch_latest_wecar_data():
    ...
    logging.info(
        "Searching for latest available WECAR data starting from %s", latest_available
    )
    for i in range(3):
...
def generate_pdf_report(data):
    ...
    story.append(
        Paragraph(f"<b>Period:</b> {data.get('report_period', 'N/A')}", styles["Normal"])
    )
...
def get_historical_data():
    ...
    if not start_str or not end_str:
        return (
            jsonify(
                {
                    "error": "Please provide both 'start_date' and 'end_date' in YYYY-MM format."
                }
            ),
            400,
        )
    try:
        ...
        return (
            jsonify({"error": "Invalid date format. Please use YYYY-MM."}),
            400,
        )
    ...
    weighted_avg_price = (
        (df["key_metrics.average_price"] * df["key_metrics.total_sales"]).sum()
        / total_sales
        if total_sales > 0
        else 0
    )
    sales_by_type_agg = df.groupby("name")["sales"].sum().reset_index().to_dict(
        "records"
    )
    ...
    if excluded_months:
        ...
        aggregated_result["message"] = (
            "Recent months were excluded due to publication lag."
        )
